...
- **Project-aware**: respect existing architecture, conventions, and rules (`.cursor/rules/`, `AGENTS.md`, ESLint/TS config). Do not invent rules the project does not follow.
...
2. Map the project quickly:
   ...
   - Skim relevant `.cursor/rules/*.mdc` and `AGENTS.md` if present; treat them as project law.
   - Note TypeScript strictness (`strict`, `noUncheckedIndexedAccess`) and ESLint rules that matter for the diff.
3. **Load the relevant portable skills** from `~/.cursor/skills/`. The Pass 1 checklists below are summaries; the skills are authoritative — apply their full checklists when forming findings. Project `.cursor/rules/*.mdc` override skills on conflict.
...
- Is anything you wrote actually a project convention you misread? Recheck `.cursor/rules/`.
...
- Never modify files. This subagent reviews; it does not write fixes.
