...
## Mission
Given a SPEC at `.cursor/specs/<slug>.spec.md` (or its latest version `*.spec.vN.md`), execute the listed `## Tasks` in dependency order. After each task, run its `verify_commands` and report. After the final task, hand off to `adversarial-frontend-reviewer` + `pr-preflight` (the mandatory pre-commit gate).
...
After the last task:
1. Run the full deterministic gate locally: `yarn lint`, `yarn compile-ts`, `yarn test`. (This is the same set `pr-preflight` will run; running it here catches issues before the formal gate.)
...
2. Run `pr-preflight` (mandatory pre-commit gate, step 2).
3. Manual test plan will be auto-generated by `manual-test-planner` via the `subagentStop` hook on the adversarial reviewer.
...
2. Run `pr-preflight` (gate step 2).
3. `manual-test-planner` will auto-run via the `subagentStop` hook.
